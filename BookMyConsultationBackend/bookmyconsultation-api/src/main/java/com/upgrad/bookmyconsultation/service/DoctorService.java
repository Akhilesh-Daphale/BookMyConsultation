...
public class DoctorService {
	...
	public Doctor getDoctor(String doctorId) {
		return Optional.ofNullable(doctorRepository.findById(doctorId)).get()
	...
	public TimeSlot getTimeSlots(String doctorId, String date) {
		...
		timeSlot.setTimeSlot(timeSlot.getTimeSlot()
				.stream()
				.filter(slot -> appointmentRepository.findByDoctorIdAndTimeSlotAndAppointmentDate(
						timeSlot.getDoctorId(), slot, timeSlot.getAvailableDate()) == null
				)
				.collect(Collectors.toList()));
